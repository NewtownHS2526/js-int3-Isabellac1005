...
if (weeklyAllowance % 3 === 0) {
    console.log(weeklyAllowance / 3);
} else {
    console.log((weeklyAllowance / 3).toFixed(2));
}
...
let quinnRicardoMoney = (weeklyAllowance / 3) * 2 * 4;
console.log( Math.floor( quinnRicardoMoney / shoes ) );
// 5. How many more games can they buy if they put their money together compared to if they had bought games on their own?
let individualGames = Math.floor( (weeklyAllowance / 3) / game ) * 3 * 4; 
let combinedGames = Math.floor( quinnRicardoMoney / game );
console.log( combinedGames - individualGames );
// 6. Print out the three siblings' names. "The three siblings are __, __, and ___."
 thethreeSiblings = `The three siblings are ${sibling1}, ${sibling2}, and ${sibling3}.`;
console.log(thethreeSiblings); 
// 7. Use console.log() and a built-in method to count how many characters are in Ricardo's name.
console.log(siblingl3.length); 
...
console.log( sibling3.replaceAll( 'a', '' ).replaceAll( 'A', '' ) );

// 11. Use console.log() and a built-in method to print out "De La Rosa"
console.log( sibling3.slice( 16, 25 ) );
...
const isBeyonce = (sofia) => {
    if( ) { 
        return "Welcome Queen!";
    } else {
        return `Sorry {Sofia}, you're not Beyonce`;
    }
}
